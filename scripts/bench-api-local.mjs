/**
 * Phase 5 — local API micro-benchmark (PGlite / createTestApp).
 * Never point this at Railway production.
 *
 * Usage: node --import tsx scripts/bench-api-local.mjs
 * or:    npm --prefix apps/api exec -- tsx ../../scripts/bench-api-local.mjs
 */
...
import {
  ...
  startTestDatabase,
  stopTestDatabase,
} from '../apps/api/tests/helpers/test-db.ts';
...
async function timeMany(
  n: number,
  fn: () => Promise<Response>,
): Promise<{ p50: number; p95: number; errors: number; samples: number[] }> {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    try {
      const res = await fn();
      if (!res.ok && res.status >= 500) errors += 1;
      if (res.status >= 400 && res.status !== 401) {
        // count unexpected client errors for bench endpoints that should succeed
      }
      if (!res.ok) errors += 1;
    } catch {
      errors += 1;
...
async function main() {
  ...
  const user = await registerLogin(app, 'bench@example.com', password);
  await app.request('/organization', {
    ...
    body: JSON.stringify({ name: 'Bench Clinic' }),
  });
  ...
  for (const n of sizes) {
    await resetDatabase(prisma);
    const u = await registerLogin(app, `bench-${n}@example.com`, password);
    await app.request('/organization', {
      method: 'POST',
      headers: {
        ...authHeader(u.token),
    ...
    await Promise.all(
      paths.map((path) =>
        app.request(path, { headers: authHeader(u.token) }),
      ),
    );
    hydrate.push({
      patients: n,
  ...
  console.log(JSON.stringify({ latency: rows.map(({ endpoint, p50, p95, errors }) => ({ endpoint, p50, p95, errors })), hydrate }, null, 2));
